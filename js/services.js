...
.factory('flightStatApi', ['$http', function($http){
  ...
  var getFlights = function(airport,type,date){
    var queryUrl =baseUrl + "flightstatus/rest/v2/jsonp/airport/status/LAS/arr/2014/05/23/14" + "?appId="+options.appId+"&appKey=" +options.appKey+ "&callback=JSON_CALLBACK";
    return $http.jsonp(queryUrl);
  };

  return {
    airports: function(){return getAirports();},
    flights: function(){return getFlights();},
    };
}]);